...
class BlinksResponse(BaseModel):
    blink_count:       int
...
class GazeResponse(BaseModel):
    eye_contact:   bool
    contact_ratio: float           # 0–1 rolling average
...
@app.get("/metrics/engagement", response_model=EngagementResponse, tags=["Metrics"])
def metric_engagement():
    """Engagement score [0–100] with colour indicator."""
    _require_running()
    r = state.get()
    return EngagementResponse(
        engagement_score = r.engagement_score,
        color            = _score_color(r.engagement_score),
    )


@app.get("/metrics/confusion", response_model=ConfusionResponse, tags=["Metrics"])
def metric_confusion():
    """Confusion score [0–100] with colour indicator."""
    _require_running()
    r = state.get()
    return ConfusionResponse(
        confusion_score = r.confusion_score,
        color           = _score_color(100 - r.confusion_score),   # inverted
    )


@app.get("/metrics/ear", response_model=EARResponse, tags=["Metrics"])
def metric_ear():
    """Eye Aspect Ratio values — left, right, average + blink/sleep state."""
    _require_running()
    r = state.get()
    return EARResponse(
        ear_left    = r.ear_left,
...
@app.get("/metrics/blinks", response_model=BlinksResponse, tags=["Metrics"])
def metric_blinks():
    """Total blink count and blinks per minute."""
    _require_running()
    r = state.get()
    return BlinksResponse(
        blink_count       = r.blink_count,
        blinks_per_minute = r.blinks_per_minute,
    )


@app.get("/metrics/head", response_model=HeadResponse, tags=["Metrics"])
def metric_head():
    """Head tilt angle in degrees and tilt alert state."""
    _require_running()
    r = state.get()
    return HeadResponse(
        tilt_angle_deg = r.tilt_angle_deg,
        is_tilted      = r.is_tilted,
        smoothed_angle = r.tilt_angle_deg,
    )


@app.get("/metrics/gaze", response_model=GazeResponse, tags=["Metrics"])
def metric_gaze():
    """Eye contact state and rolling contact ratio [0–1]."""
    _require_running()
    r = state.get()
    return GazeResponse(
        eye_contact   = r.eye_contact,
...
@app.get("/metrics/gesture", response_model=GestureResponse, tags=["Metrics"])
def metric_gesture():
...
@app.get("/metrics/system", response_model=SystemResponse, tags=["Metrics"])
def metric_system():
    """System performance — FPS, inference time, frame count."""
    r = state.get()
